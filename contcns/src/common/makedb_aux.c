...
{
	kstring_t* name = new_kstring();
	make_reads_info_name(wrk_dir, name);
	DFOPEN(out, KSTR_DATA(*name), "w");
...
load_num_volumes(const char* wrk_dir)
{
	kstring_t* name = new_kstring();
	make_reads_info_name(wrk_dir, name);
	DFOPEN(in, KSTR_DATA(*name), "r");
	int num_volumes;
	fscanf(in, "%d", &num_volumes);
	FCLOSE(in);
	free_kstring(name);
	return num_volumes;
}
...
load_num_reads(const char* wrk_dir)
{
	...
	fscanf(in, "%d%d", &num_volumes, &num_reads);
	FCLOSE(in);
	free_kstring(name);
	return num_reads;
}
...
vi_volume_name(VolumesInfo* vi, int vid)
{
	...
	return KSTR_DATA(vi->volume_names) + vn_start;
}
...
load_volumes_info(const char* wrk_dir)
{
	...
	kstring_t* vnn = new_kstring();
	char line[2048];
	size_t vn_start = 0;
	make_volume_name_name(wrk_dir, vnn);
	DFOPEN(in, KSTR_DATA(*vnn), "r");
