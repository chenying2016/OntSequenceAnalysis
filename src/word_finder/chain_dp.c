...
chain_dp(ChainDpData* data, const int qid, const int qdir, const idx qsize, const int sid, const idx ssize)
{
	...
	//OC_LOG("number of seeds: %lu", n_seeds);
	for (i = 0; i < n_seeds; ++i) {
		idx ri = seeds[i].soff;
		idx qi = seeds[i].qoff;
		int max_j = -1;
		...
		for (j = i - 1; j >= st; --j) {
			...
			idx dq = (qi > seeds[j].qoff) ? (qi - seeds[j].qoff) : (seeds[j].qoff - qi);
			if (dq > max_dist) continue;
			idx dd = dr > dq ? dr - dq : dq - dr;
			if (dd > bw) continue;
			idx min_d = dr > dq ? dq : dr;
			int sc = min_d;
	...
	ks_introsort(GappedCandidate_ScoreGT, kv_size(*lcanv), kv_data(*lcanv));
	int m = kv_size(*lcanv);
	for (i = 0; i < m; ++i) {
		if (kv_A(*lcanv, i).qid == -1) continue;
		idx iqbeg = kv_A(*lcanv, i).qbeg;
		idx iqend = kv_A(*lcanv, i).qend;
		idx isbeg = kv_A(*lcanv, i).sbeg;
		idx isend = kv_A(*lcanv, i).send;
		for (j = i + 1; j < m; ++j) {
			if (kv_A(*lcanv, j).qid == -1) continue;
			idx jqoff = kv_A(*lcanv, j).qoff;
			idx jsoff = kv_A(*lcanv, j).soff;
			if (jqoff >= iqbeg && jqoff <= iqend) {
				kv_A(*lcanv, j).qid = -1;
				continue;
			}
			if (jsoff >= isbeg && jsoff <= isend) {
				kv_A(*lcanv, j).qid = -1;
				continue;
			}
		}
	...
	i = 0;
	for (j = 0; j < m; ++j) {
		if (kv_A(*lcanv, j).qid != -1) {
			kv_A(*lcanv, i) = kv_A(*lcanv, j);
			++i;
		}
	}
	kv_resize(GappedCandidate, *lcanv, i);
}
